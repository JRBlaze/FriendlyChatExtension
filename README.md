...
1. **Get the folder onto your computer.** If you downloaded a ZIP, unzip it first. You should end
   up with a folder that has `manifest.json` sitting directly inside it, next to `src` and `icons`.
...
5. **Choose the folder from step 1** and click Select Folder. Pick the folder *itself*, not
   `manifest.json` and not a folder above it.
